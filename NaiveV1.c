...
int binarySearch(long lower, long upper, char *ciph, int len) {
    ...
    while (left <= right) {
        ...
        // Ajusta los límites para la siguiente iteración de búsqueda binaria
        if (found == 0) {
            if (mid < found) {
                left = mid + 1;
            } else {
...
int main(int argc, char *argv[]){
    ...
    if (id == 0) {
        ...
        // Guarda el texto encriptado en un archivo (por ejemplo, "encryptednaiveV1.txt")
        if (!saveTextToFile("encryptednaiveV1.txt", text, textLength)) {
            free(text);
            MPI_Finalize();
            return 1;
        }
    ...
    if (id == 0) {
        ...
        // Guarda el texto desencriptado en un archivo (por ejemplo, "decryptednaivev1.txt")
